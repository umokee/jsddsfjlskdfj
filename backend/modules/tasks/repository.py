...
class TaskRepository:
    ...
    def get_missed_habits(
        ...
        """
        Get habits that were due in the time range but not completed.

        A habit is considered missed if:
        - It's a habit (is_habit=True)
        - It was due during the time range (due_date or start_date within range)
        - It was NOT completed during the time range
        """
        return self.db.query(Task).filter(
            and_(
                Task.is_habit == True,
    ...
    def count_habits_due_in_range(
        ...
        """Count habits that were due in the time range."""
        return self.db.query(Task).filter(
            and_(
                Task.is_habit == True,
                or_(
